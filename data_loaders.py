...
'''
    def _get_file_list(self, cfg, subset, n_renderings=1):
        ...
        for dc in self.dataset_categories:
            ...
            for s in tqdm(samples, leave=False):
                # .pcd のパスを指定（拡張子を .pcd に合わせる）
                gt_path = cfg.DATASETS.CUSTOM.COMPLETE_POINTS_PATH % (dc['taxonomy_id'], s)
                ...
                if subset == 'test':
                    for i in range(n_renderings):
                        file_list.append({
                            ...
                            'model_id': s,
                            'partial_cloud_path': cfg.DATASETS.CUSTOM.PARTIAL_POINTS_PATH % (dc['taxonomy_id'], s),
                    ...
                    for i in range(n_renderings):
                        partial_paths.append(
                            cfg.DATASETS.CUSTOM.PARTIAL_POINTS_PATH % (dc['taxonomy_id'], s)
                        )
        ...
        return file_list
'''
# data_loaders.py の CustomDataLoader クラス内
def _get_file_list(self, cfg, subset, n_renderings=1):
        file_list = []
        for dc in self.dataset_categories:
            logging.info('Collecting Custom files: [ID=%s]' % (dc['taxonomy_id']))
            ...
            for s in tqdm(samples, leave=False):
                # 【修正点1】 Completeパスの生成
                # 設定ファイルの %s が1つなので、変数 s (モデルID) だけを渡す
                ...
                if subset == 'test':
                    ...
                    for i in range(n_renderings):
                        file_list.append({
                            'taxonomy_id': dc['taxonomy_id'],
                            'model_id': s,
                            # 【修正点2】 Partialパスの生成 (Test)
                            # 設定ファイルの %02d が1つなので、変数 i (視点番号) だけを渡す
                            'partial_cloud_path': cfg.DATASETS.CUSTOM.PARTIAL_POINTS_PATH % i,
                            'gtcloud_path': gt_path
                        })
                else:
                    # 学習時 (Train/Val)
                    partial_paths = []
                    for i in range(n_renderings):
                        # 【修正点3】 Partialパスの生成 (Train)
                        # ここも変数 i だけを渡す
                        partial_paths.append(
                            cfg.DATASETS.CUSTOM.PARTIAL_POINTS_PATH % i
                        )
