...
def run_inference():
    ...
    # --------------------------------------------------------------------------
    # 4) パッチ中心の生成 (★前のプログラムのZスライス方式を統合)
    # --------------------------------------------------------------------------
    print("Generating centers using Z-Slice logic...")
    centers_list = []
    
    # Z座標の最小・最大を取得
    z_min = points_full[:, 2].min()
    z_max = points_full[:, 2].max()
    
    # Z軸を下から上へ STRIDE ずつループ
    z_steps = np.arange(z_min, z_max, CENTER_STRIDE)
    
    print(f"Slicing Z-axis from {z_min:.3f} to {z_max:.3f} (steps: {len(z_steps)})")

    # Zループ
    for z in tqdm(z_steps, desc="Generating Grid"):
        # 現在の高さ(z)付近にある点だけを抽出 (厚さは PATCH_SIZE 程度を見る)
        # これにより、点群が存在する場所だけ中心点を作ることができます
        z_mask = np.abs(points_full[:, 2] - z) < (PATCH_SIZE / 2.0)
        layer_points = points_full[z_mask]

        if len(layer_points) < 10:
            continue

        # この層(layer)に対して2D的なグリッドを作るためにVoxel Downsampleを使用
        pcd_layer = o3d.geometry.PointCloud()
        pcd_layer.points = o3d.utility.Vector3dVector(layer_points)
        
        # 層内でのダウンサンプリング (XY平面上の分布を取得)
        pcd_layer_ds = pcd_layer.voxel_down_sample(voxel_size=CENTER_STRIDE)
        layer_centers = np.asarray(pcd_layer_ds.points).astype(np.float32)
        
        # 重要: 中心の高さはループ変数の 'z' に強制統一する
        # (こうすることで、綺麗な層ごとのスキャンになります)
        layer_centers[:, 2] = z
        
        centers_list.append(layer_centers)

    if not centers_list:
        print("Error: No centers generated.")
        return

    centers = np.vstack(centers_list)
    n_patches = centers.shape[0]
    print(f"Generated {n_patches} patch centers via Z-slicing.")
    ...
    # TQDMを使って進捗表示
    for i in tqdm(range(len(centers)), desc="Inference"):
