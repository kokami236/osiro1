...
parser.add_argument(
    "--desc", type=str, default="Training/Testing SeedFormer", help="description"
)
parser.add_argument("--net_model", type=str, default="model", help="Import module.")
parser.add_argument(
    "--arch_model", type=str, default="seedformer_dim128", help="Model to use."
)
parser.add_argument(
    "--test", dest="test", help="Test neural networks", action="store_true"
)
parser.add_argument(
    "--inference", dest="inference", help="Inference for benchmark", action="store_true"
)
parser.add_argument("--output", type=int, default=False, help="Output testing results.")
parser.add_argument(
    "--pretrained", type=str, default="", help="Pretrained path for testing."
)
...
def PCNConfig():
    ...
    __C.DATASETS.COMPLETION3D.PARTIAL_POINTS_PATH = (
        "/path/to/datasets/Completion3D/%s/partial/%s/%s.h5"
    )
    __C.DATASETS.COMPLETION3D.COMPLETE_POINTS_PATH = (
        "/path/to/datasets/Completion3D/%s/gt/%s/%s.h5"
    )
    ...
    __C.DATASETS.SHAPENET.PARTIAL_POINTS_PATH = "/home/limu/seedformer-master/My_PCN_Dataset-20251204T033849Z-1-001/My_PCN_Dataset/shapenet_pc/02691156/partial/%s.pcd"
    __C.DATASETS.SHAPENET.COMPLETE_POINTS_PATH = "/home/limu/seedformer-master/My_PCN_Dataset-20251204T033849Z-1-001/My_PCN_Dataset/shapenet_pc/02691156/complete/%s.pcd"

    __C.DATASETS.CUSTOM = edict()
    __C.DATASETS.CUSTOM.CATEGORY_FILE_PATH = "/home/limu/seedformer-master/My_PCN_Dataset-20251204T033849Z-1-001/My_PCN_Dataset/Custom.json"
    ...
    __C.DATASET.TRAIN_DATASET = "Custom"
...
def train_net(cfg):
    ...
    val_data_loader = torch.utils.data.DataLoader(
        dataset=val_dataset_loader.get_dataset(utils.data_loaders.DatasetSubset.VAL),
        batch_size=cfg.TRAIN.BATCH_SIZE,
        num_workers=cfg.CONST.NUM_WORKERS // 2,
        collate_fn=utils.data_loaders.collate_fn,
        pin_memory=True,
...
def test_net(cfg):
    ...
    if args.pretrained == "":
        ...
        list_pretrained = [
            train_name
            for train_name in list_trains
            if train_name.startswith(TRAIN_NAME + "_Log")
        ]
        if len(list_pretrained) != 1:
            raise ValueError(
                "Find {:d} models. Please specify a path for testing.".format(
                    len(list_pretrained)
                )
            )
        cfg.DIR.PRETRAIN = list_pretrained[0]
        cfg.CONST.WEIGHTS = os.path.join(
            cfg.DIR.OUT_PATH, cfg.DIR.PRETRAIN, "checkpoints", "ckpt-best.pth"
        )
    ...
    # Start training
    manager.test(
        cfg, model, val_data_loader, outdir=cfg.DIR.RESULTS if args.output else None
    )
