...
==============================================================
    ...
    Functions for point cloud augmentation

==============================================================
-*- coding: utf-8 -*-
@Author: Haozhe Xie
@Date:   2019-08-02 14:38:36
@Last Modified by: Haoran Zhou
@Last Modified time:
==============================================================
'''
                ...
                for k, v in data.items():
                    if k in objects and k in data:
                        if transform.__class__ in [
                                RandomCrop, RandomFlip, RandomRotatePoints,
                                ScalePoints, RandomMirrorPoints
                        ]:
...
class CenterCrop(object):
    ...
    def __call__(self, img):
        ...
        # Crop the image
        img = cv2.resize(
            img[int(y_top):int(y_bottom),
                int(x_left):int(x_right)], (self.img_size_w, self.img_size_h))
...
class RandomBackground(object):
    ...
    def __call__(self, img):
        ...
        r, g, b = [
            np.random.randint(self.random_bg_color_range[i][0],
...
class RandomClipPoints(object):
    ...
    def __call__(self, ptcloud):
        ptcloud += np.clip(self.sigma * np.random.randn(*ptcloud.shape),
...
class RandomRotatePoints(object):
    ...
    def __call__(self, ptcloud, rnd_value):
        ...
        trfm_mat = np.dot(transforms3d.axangles.axangle2mat([0, 1, 0], angle),
                          trfm_mat)
